...
//const ages = [22, 14, 24, 55, 65, 21, 12, 13, 90];
const filteredAges2 = ages.filter((age) => age % 2 === 0);
...
const filteredStreamers2 = streamers.filter((streamer) => streamer.name.includes('u'));
...
const filteredStreamers3 = streamers.filter((streamer) => streamer.gameMorePlayed.includes('Legends'));
filteredStreamers3.forEach(element => {
  if (element.age > 35) {
    let inUpper = element.gameMorePlayed.toUpperCase()
    element.gameMorePlayed = inUpper
  };
});
